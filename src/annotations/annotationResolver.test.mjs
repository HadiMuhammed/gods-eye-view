...
test('a not-found is remembered only when every source consulted gave a verdict', () => {
  ...
  for (const [sources, expected] of cases) {
    assert.equal(geocodeMissIsDefinitive(sources), expected, JSON.stringify(sources));
  }
  // An omitted Google verdict defaults to "did not say no such place".
  assert.equal(geocodeMissIsDefinitive({ googleConsulted: true, photonAnswered: true }), false);
});
