...
import { createOpenSkySource, createAdsbLolSource, createAisStreamSource, normalizeReadsbAircraft, normalizeVesselObservation, openSkySnapshot, readsbSnapshot } from './index.js';

const now = 1800000000000;
const aircraft = ['AbC123', 'TEST', 'Example', now / 1000 - 15, now / 1000 - 2, -97, 30, 1000, false, 100, 250, -2, null, 1040];
const response = (payload, headers = {}, status = 200) => Response.json(payload, { headers, status });

test('civil observations retain identity, separate altitude datums and source epochs', () => {
  const snapshot = openSkySnapshot({ time: now / 1000 - 130, states: [aircraft] }, { now, source: 'Test source', coverage: 'regional' });
  ...
  const missing = openSkySnapshot({ states: [[...aircraft.slice(0, 3), null, null, ...aircraft.slice(5)]] }, { now });
...
test('mixed-invalid and duplicate admissions are explicitly incomplete; all-invalid rejects', () => {
  const invalid = [...aircraft]; invalid[5] = 181;
  ...
  assert.throws(() => openSkySnapshot({ states: [invalid] }), { code: 'malformed' });
...
test('readsb units and position/contact ages are normalized once against the source epoch', () => {
  const row = normalizeReadsbAircraft({ hex: 'abc123', lat: '30', lon: '-97', alt_baro: 10000, alt_geom: 10100, gs: 120, baro_rate: -600, seen: 2, seen_pos: 12 }, now);
  ...
  assert.equal(row.contactTimeMs, now - 2000);
  const ground = normalizeReadsbAircraft({ hex: 'abc123', lat: 30, lon: -97, alt_baro: 'ground' }, null);
  assert.equal(ground.onGround, true);
  assert.equal(ground.baroAltitudeM, null);
  assert.equal(ground.positionTimeMs, null);
  assert.equal(readsbSnapshot({ ac: [] }, { observedAtMs: null }).freshness, 'unknown');
});

test('vessel identity, opaque reference and heading/course remain distinct, unknowns stay null', () => {
  const row = normalizeVesselObservation({ mmsi: '123456789', lat: 30, lon: -97, speed: 10, heading: 45, course: 60, last_position_epoch: now / 1000, extra: 'discard' }, 'opaque-reference');
  ...
  assert.equal(normalizeVesselObservation({ mmsi: 'x', lat: null, lon: null }), null);
});
...
test('construction is inert; adapters preserve routes, viewport query, cache epoch and history datum', async () => {
  ...
  const fetchImpl = async (url, init) => {
    ...
    if (url.startsWith('/api/opensky?')) return response({ time: now / 1000, states: [aircraft] }, { 'x-flight-source': 'adsb.lol', 'x-flight-coverage': '250 nm regional' });
    if (url.startsWith('/api/opensky-track')) return response({ path: [[now / 1000 - 30, 30, -97, 1000, 45, false]] });
    if (url === '/api/adsblol/mil') return response({ ac: [{ hex: 'abc123', lat: 30, lon: -97, seen_pos: 15 }] }, { 'x-ads-b-cache-age-ms': '60000', 'x-ads-b-cache': 'STALE' });
    return response({ timestamp: now / 1000 - 60, trace: [[10, 30, -97, 1000]] });
  };
  const civil = createOpenSkySource({ fetchImpl, now: () => now });
  const military = createAdsbLolSource({ fetchImpl, now: () => now });
  assert.equal(requests.length, 0);
  const signal = new AbortController().signal;
  const snapshot = await civil.getSnapshot({ latitude: 30.123456, longitude: -97 }, { signal });
...
test('cancellation after slow body parsing rejects even with a transport that ignores abort', async () => {
  ...
  const source = createOpenSkySource({ fetchImpl: async () => ({ ok: true, json: () => new Promise(resolve => { release = resolve; }) }) });
...
test('denials and outages never start another source and do not echo arbitrary response bodies', async () => {
  for (const status of [401, 403, 429, 502]) {
    let calls = 0;
    const source = createOpenSkySource({ fetchImpl: async () => { calls++; return response({ error: 'sensitive upstream detail' }, {}, status); } });
...
test('AIS reports limited received coverage and keeps connection state separate from positions', async () => {
  const source = createAisStreamSource({ origin: () => 'http://example.test', fetchImpl: async url => {
    if (url.includes('/track?')) return response({ samples: [{ lat: 30, lon: -97, t: now / 1000 }] });
    assert.equal(url, 'http://example.test/api/ais-live?maxRows=500');
    return response({ status: 'reconnecting', refreshing: true, rows: [{ mmsi: '123456789', lat: 30, lon: -97, last_position_UTC: new Date(now).toISOString() }], newestPositionAt: new Date(now).toISOString() });
  } });
  ...
  assert.equal((await source.getTrack('123456789')).records[0].observedAtMs, now);
});
