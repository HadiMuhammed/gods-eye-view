...
import { cockpitAnchorCorrectionStep, cockpitGroundSafeHeight, cockpitSurfaceWaitExpired, cockpitUiUpdateDue, slewHeading } from '../cockpitMath.js';
import { COCKPIT_HEADING_SLEW_DPS, COCKPIT_FORWARD_OFFSET_M, COCKPIT_UP_OFFSET_M, COCKPIT_MIN_GROUND_CLEARANCE_M, COCKPIT_VIEW_PITCH_DEG, COCKPIT_CAMERA_UPDATE_MS, COCKPIT_HUD_UPDATE_MS, COCKPIT_GROUND_PROBE_MS, COCKPIT_GROUND_WAIT_TIMEOUT_MS } from './cockpitPresentation.js';
...
export function update() {
    ...
    let cockpitFloorM = this.services.cachedGroundFloor(info.latitude, info.longitude);
    if (info.onGround === true) {
      const groundPoint = [{ lat: info.latitude, lon: info.longitude }];
      this.services.warmGroundFloor(groundPoint);
      const meshFloorM = this.services.cachedMeshFloor(info.latitude, info.longitude);
      if (this.services.meshFloorPreferred() && !Number.isFinite(meshFloorM)) {
        ...
        cockpitFloorM = this.services.cachedMeshFloor(info.latitude, info.longitude);
    ...
    if (!Number.isFinite(cockpitFloorM)
      ...
      cockpitFloorM = targetCartographic.height - this.services.GROUND_FLOOR_LIFT_M;
    }
