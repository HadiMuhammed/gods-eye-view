...
import { normalizeHeading, resolveTrackedAircraftInfo } from '../cockpitMath.js';
...
export function toggleTrackedTr3b() {
    ...
    const layer = info.layerId === 'military' ? this.services.militaryFlightsLayer : this.services.flightsLayer;
...
export function navigateContext(direction, options = {}) {
    ...
    const changed = Boolean(this.services.militaryAwarenessLayer?.[method]?.(navigationOptions));
    if (!changed) {
