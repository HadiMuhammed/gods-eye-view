...
export class CockpitDisplayPortal {
  constructor({ standardPanel, cockpitPanel, groups, layout }) {
    ...
    this.records = groups.flatMap(([name, group]) => {
      const slot = cockpitPanel?.querySelector(`[data-cockpit-display-slot="${name}"]`);
    ...
    standardPanel?.addEventListener('scroll', () => {
      if (!this.active) this.standardScrollTop = standardPanel.scrollTop;
    }, options);
    cockpitPanel?.addEventListener('scroll', () => {
  ...
  setActive(active, { settle = true } = {}) {
    ...
    const focusedRecord = this.records.find(record => record.group.contains(document.activeElement));
    ...
    this.cockpitPanel?.classList.toggle('uses-shared-display-controls', nextActive);
    const restoreScroll = () => {
      if (nextActive && this.cockpitPanel) this.cockpitPanel.scrollTop = this.cockpitScrollTop;
      if (!nextActive && this.standardPanel) this.standardPanel.scrollTop = this.standardScrollTop;
    };
    if (!settle) { restoreScroll(); this.restoreOwner = null; return; }
