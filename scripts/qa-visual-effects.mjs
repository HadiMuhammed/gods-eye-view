...
try {
  ...
  check(
    ...
    await page.evaluate(() => {
      const manager = window.__godsEyeView.styleManager;
      const states = [];
      const unsubscribe = manager.subscribeShareState((value) => states.push(value));
      ...
      return (
        states[0].initial && states.length > 1 && Object.isFrozen(published.state.options) &&
        published.state.bloomEnabled && published.state.options.bloomIntensity === 98 &&
        result.ok &&
